...
try {
    ...
    console.error("CRITICAL: ARWEAVE_KEY missing in .env");
}
...
// --- 1. DYNAMIC WEIGHTED AGENTS (Updated for GitHub Data) ---
const Agents = {
    technical: (desc, githubData) => {
        ...
        // Dynamic boost based on real GitHub metrics
        if (githubData) {
            score += 5; // Base boost for having a verified repo
            if (githubData.stars > 10) score += 5;
            if (githubData.stars > 100) score += 3;
        } else {
            score -= 10; // Penalty for missing/invalid repo
        }
...
const saveReviewToArweave = async (reviewData) => {
    try {
        ...
        console.log(`Arweave Audit ID: ${receipt.id}`);
        return receipt.id;
    } catch (err) {
...
app.post('/api/review', async (req, res) => {
    ...
    // STEP 1: Fetch live external verification data
    const githubData = await verifyGitHub(githubUrl);

    // STEP 2: Pass verification data to Technical Agent
    const swarmResult = {
    ...
    const stateProof = crypto.createHash('sha256')
        .update(JSON.stringify(swarmResult) + Date.now())
        .digest('hex');

    const arweaveId = await saveReviewToArweave({
        title, budget, githubUrl,
        githubMeta: githubData, // Anchor the live stats at time of review
...
const PORT = 8000;
app.listen(PORT, () => {
    console.log(`SWARM ORCHESTRATOR ACTIVE | Port: ${PORT}`);
    if(walletKey) {
        const arweave = Arweave.init({ host: 'arweave.net', port: 443, protocol: 'https' });
        arweave.wallets.jwkToAddress(walletKey).then(addr => console.log(`Swarm Wallet: ${addr}`));
    }
});